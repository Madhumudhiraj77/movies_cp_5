...
app.get("/movies/", async (request, response) => {
  ...
  const allMovies = await db.all(getMovies);
  response.send(
    allMovies.map((eachMovie) => ({ movieName: eachMovie.movie_name }))
  );
});
...
app.get("/movies/:movieId/", async (request, response) => {
  ...
  response.send(movieObject(movie));
});
...
app.get("/directors/", async (request, response) => {
  ...
  response.send(
    directorsArray.map((eachDirector) => directorObject(eachDirector))
  );
});
